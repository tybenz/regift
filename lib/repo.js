(function() {
  ...
  module.exports = Repo = (function() {
    ...
    Repo.prototype.sync = function(remote_name, branch, callback) {
      var _ref1, _ref2,
        _this = this;
      if (!callback) {
        _ref1 = ['origin', branch, remote_name], remote_name = _ref1[0], callback = _ref1[1], branch = _ref1[2];
      }
      if (!branch) {
        _ref2 = ['origin', remote_name, []], remote_name = _ref2[0], callback = _ref2[1], branch = _ref2[2];
      }
      return this.git("stash", {}, ["save"], function(err) {
        if (err) {
          return callback(err);
        }
        return _this.git("pull", {}, [remote_name, branch], function(err) {
          if (err) {
            return callback(err);
          }
          return _this.git("push", {}, [remote_name, branch], function(err) {
            if (err) {
              return callback(err);
            }
